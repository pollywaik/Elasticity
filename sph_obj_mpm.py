...
class GridMpm:
    def __init__(self, grid_num):
        ...
        ti.root.dense(ti.i, (grid_num,) * dim).place(self.vel)
        ti.root.dense(ti.i, (grid_num,) * dim).place(self.mass)
...
class Fluid:
    def __init__(self, max_part_num):
        ...
        self.general_flag = ti.field(int, ())  # ? OBSOLETE
        self.pushed_part_seq = ti.Vector.field(dim, int, ())  # ? OBSOLETE
        self.pushed_part_seq_coder = ti.field(int, dim)  # ? OBSOLETE
