...
def node_construct(self, dim, node_num, capacity_list):
    for capacity in capacity_list:
        ...
        if capacity == 'node_elasticity':
            self.elasticity = struct_node_elasticity(dim, node_num)
        if capacity == 'node_elasticity_neighbor':
            self.elasticity_neighbor = struct_node_elasticity_neighbor(node_num)
        if capacity == 'node_conjugate_gradient':
            self.conjugate_gradient = struct_node_conjugate_gradient(dim, node_num)
...
def info_construct(self, dim, capacity_list):
    for capacity in capacity_list:
        ...
        if capacity == 'info_elasticity':
            self.elasticity = struct_config_elasticity()